...
test_corrupted_manifest_recovery :: proc(t: ^testing.T) {
    ...
    temp_dir := setup_test_environment("test_temp_corrupted")
...
`
    ...
    defer delete(valid_manifest_path)
    ...
    corrupted_shell_path := filepath.join({corrupted_dir, "test.zsh"})
    defer delete(corrupted_shell_path)
    valid_shell_path := filepath.join({valid_dir, "valid.zsh"})
    defer delete(valid_shell_path)
    os.write_entire_file(corrupted_shell_path, transmute([]u8)shell_content)
...
test_permission_denied_recovery :: proc(t: ^testing.T) {
    ...
    temp_dir := setup_test_environment("test_temp_permissions")
    defer teardown_test_environment(temp_dir)
    
    // Create accessible module
    accessible_dir := filepath.join({temp_dir, "accessible-module"})
    defer delete(accessible_dir)
...
`, i, i, i * 10, i)
        ...
        defer delete(dep_manifest_path)
...
test_recursive_directory_error_handling :: proc(t: ^testing.T) {
    ...
    temp_dir := setup_test_environment("test_temp_deep_nesting")
    defer teardown_test_environment(temp_dir)
    
    // Create deeply nested structure (within scanner max depth of 10)
    current_path := temp_dir
    for i in 1..=9 {
        next_path := filepath.join({current_path, fmt.tprintf("level%d", i)})
